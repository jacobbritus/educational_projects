...
class ToDoList:
    def __init__(self, box_length = 50):
        ...
        self.key_space = 10
        self.check_space = 10
        self.name_space = box_length - (self.key_space + self.check_space)
    ...
    def display_tasks(self, key) -> None:
        ...
        print(display_format.format(key="Key".center(self.key_space),
                                    task="Task".center(self.name_space),
        ...
        if key == "edit_keybinds":
            for i in range(TASK_PAGE_CAP):
                ...
                print(format_box(BoxVariant.C_SIDE, self.box_length, row="╋", list = self))

                print(display_format.format(key=keybind.center(key_center),
                                            task=" ".center(self.name_space),
                                            status=" ".center(self.check_space)))

            print(format_box(BoxVariant.BOTTOM, self.box_length, row="┻", list = self))
            return
    ...
    def adapt_box_length(self) -> None:
        """Scales the UI width based on name lengths"""
        for task in list(self.task_info.keys()):
            print(len(task))
            if len(task) > self.name_space:
                self.box_length += len(task) - self.name_space
                self.name_space += len(task) - self.name_space

        if all(len(task) < 50 for task in list(self.task_info.keys())):
            change = self.box_length - 50
            self.box_length = 50
            self.name_space -= change

    def remove_tasks(self, key) -> None:
    ...
    def add_tasks(self, task_name) -> None:
        """Adds the task with the name passed in as a parameter and saves the change."""
        self.task_info.update({task_name: {"status": "[ ]"}})
        save_file((self.task_info, self.task_keys, self.option_keys))

    def mark_tasks(self, key) -> None:
        """Marks or unmarks the task passed as a parameter and saves the change."""
        task_keys = list(self.task_info.keys())
        task = task_keys[self.task_keys.index(key)]
    ...
    def change_keybind(self, old_key, new_key) -> None:
        """Changes a keybind."""
        ...
        elif old_key in self.option_keys:
